...
namespace EmuLib.Utils.Player
{
    internal static class HealthStateUtils
    {
        ...
        {
            var requestData = new {type = "HydrationChanged", diff};
            SendRequest(requestData);
        }

        public static void OnEnergyChangedEvent(float diff)
        {
            var requestData = new {type = "EnergyChanged", diff};
            SendRequest(requestData);
        }
        ...
        public static void OnEffectAddedEvent(PlayerStatesInterface effect)
        {
            ...
            var requestData = new
                {type = "EffectAdded", bodyPart = effect.BodyPart, effectType = effect.DisplayableVariations[0].Type};
            SendRequest(requestData);
        }
        ...
        {
            var requestData = new {type = "Died", damageType};
            SendRequest(requestData);
        }

        private static void SendRequest(object requestData)
        {
            new HttpUtils.Create(ClientAppUtils.GetSessionId())
                .Post("/player/health/events", requestData.ToJson(), true);
        }
    }
}